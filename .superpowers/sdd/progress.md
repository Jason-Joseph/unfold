...
- Task 8: complete (commit 229e26c, review false alarm — reviewer fired before implementer finished). Extended Firestore import to add collection/getDocs. Module script sets window._deckLoadPromise = getDocs(collection(db,'decks')).then().catch(()=>({})) on page load. initGame() wraps body in _deckLoadPromise.then(extra => { Object.assign(CATS, extra); ... }). Note: Jason must add Firestore rule: match /decks/{deckId} { allow read: if true; allow write: if false; }
